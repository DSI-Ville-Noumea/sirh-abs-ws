...
public abstract class AbstractAbsenceDataConsistencyRules implements IAbsenceDataConsistencyRules {
	...
	protected Logger logger = LoggerFactory.getLogger(AbsRecuperationDataConsistencyRulesImpl.class);
	...
	public ReturnMessageDto checkAgentInactivity(ReturnMessageDto srm, Integer idAgent, Date dateLundi, Demande demande) {
		...
		if (!ACTIVITE_CODES.contains(adm.getCdpadm())){
			logger.warn(String.format(ETAT_NON_PROVISOIRE_OU_SAISIE_MSG,
			demande.getIdDemande())); srm.getErrors().add(INACTIVITE_MSG); 
		}
