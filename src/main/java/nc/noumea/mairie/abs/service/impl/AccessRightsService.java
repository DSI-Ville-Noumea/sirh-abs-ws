...
public class AccessRightsService implements IAccessRightsService {
	...
	public ReturnMessageDto setOperateur(Integer idAgentAppro, AgentDto operateurDto) {
		...
		if (accessRightsRepository.isUserOperateurOfApprobateur(idAgentAppro, operateurDto.getIdAgent())) {
			logger.debug("L'agent {} est déjà opérateur de l'approbateur {}.", 
					operateurDto.getIdAgent(), idAgentAppro);
			result.getErrors().add(String.format("L'agent [%d] est déjà opérateur de l'approbateur [%d].", 
					operateurDto.getIdAgent(), idAgentAppro));
			return result;
		}
	...
	public ReturnMessageDto deleteOperateur(Integer idAgentAppro, AgentDto operateurDto) {
		...
		if (null == droitProfilOperateur) {
			logger.debug("L'agent {} n'est pas opérateur de l'approbateur {}.", 
					operateurDto.getIdAgent(), idAgentAppro);
			result.getErrors().add(String.format("L'agent [%d] n'est pas opérateur de l'approbateur [%d].", 
					operateurDto.getIdAgent(), idAgentAppro));
			return result;
		}
	...
	public ReturnMessageDto setViseur(Integer idAgentAppro, AgentDto viseurDto) {
		...
		if (accessRightsRepository.isUserViseurOfApprobateur(idAgentAppro, viseurDto.getIdAgent())) {
			logger.debug("L'agent {} est déjà viseur de l'approbateur {}.", 
					viseurDto.getIdAgent(), idAgentAppro);
			result.getErrors().add(String.format("L'agent [%d] est déjà viseur de l'approbateur [%d].", 
					viseurDto.getIdAgent(), idAgentAppro));
			return result;
		}
	...
	public ReturnMessageDto deleteViseur(Integer idAgentAppro, AgentDto viseurDto) {
		...
		if (null == droitProfilViseur) {
			logger.debug("L'agent {} n'est pas viseur de l'approbateur {}.", 
					viseurDto.getIdAgent(), idAgentAppro);
			result.getErrors().add(String.format("L'agent [%d] n'est pas viseur de l'approbateur [%d].", 
					viseurDto.getIdAgent(), idAgentAppro));
			return result;
		}
	 ...
	 * Liste des agents que l operateur saisit
	 */
	@Override
	@Transactional(readOnly = true)
	...
	/**
	 * #15688 bug cumul de rôles sous un même approbateur
	 * Liste des agents que le viseur saisit
	 */
	@Override
	@Transactional(readOnly = true)
	public List<AgentDto> getAgentsToInputByViseur(Integer idAgentApprobateur, Integer idAgentViseur) {
		return getAgentsToApproveOrInput(idAgentApprobateur, idAgentViseur, null, ProfilEnum.VISEUR);
	}
	 ...
	 */
	protected List<AgentDto> getAgentsToApproveOrInput(Integer idAgentApprobateur, Integer idAgent, String codeService, ProfilEnum profil) {
		...
		if(null == dp) {
			logger.warn("L'agent {} ne possède pas de rôle {} associé à l'approbateur {}.", idAgent,
					profil.toString(), idAgentApprobateur);
			return result;
		}
		...
		if (!idAgent.equals(idAgentOfDemande)) {
			if (accessRightsRepository.isUserOperateur(idAgent) || accessRightsRepository.isUserApprobateur(idAgent)
				...
				if (accessRightsRepository.isUserDelegataire(idAgent) && !trouve) {
					...
					for (Integer idApprobateurOfDelegataire : idsApprobateurOfDelegataire) {
						Droit droitApprobateurOfDelegataire = accessRightsRepository
								.getAgentDroitFetchAgents(idApprobateurOfDelegataire);
	...
	public List<Integer> getIdApprobateurOfDelegataire(Integer idAgentConnecte, Integer idAgentConcerne) {
		...
		if (null == idAgentConcerne) {
			...
			if (accessRightsRepository.isUserDelegataire(idAgentConnecte)) {
				List<DroitProfil> droitsProfils = accessRightsRepository.getDroitProfilByAgentAndLibelle(
						idAgentConnecte, ProfilEnum.DELEGATAIRE.toString());
				...
				if (null != droitsAgent.getDroitDroitsAgent()) {
					for (DroitDroitsAgent dda : droitsAgent.getDroitDroitsAgent()) {
						if (ProfilEnum.OPERATEUR.toString().equals(dda.getDroitProfil().getProfil().getLibelle())) {
							AgentDto operateur = new AgentDto(agentService.getAgentOptimise(listAgentsExistants, dda
									.getDroitProfil().getDroit().getIdAgent()));
						...
						if (ProfilEnum.VISEUR.toString().equals(dda.getDroitProfil().getProfil().getLibelle())) {
							AgentDto viseur = new AgentDto(agentService.getAgentOptimise(listAgentsExistants, dda
									.getDroitProfil().getDroit().getIdAgent()));
						...
						if (ProfilEnum.APPROBATEUR.toString().equals(dda.getDroitProfil().getProfil().getLibelle())) {
							AgentWithServiceDto approbateur = new AgentWithServiceDto(agentService.getAgentOptimise(
									listAgentsExistants, dda.getDroitProfil().getDroit().getIdAgent()));
							...
							if (!result.getListApprobateurs().contains(approbateurWithDelegataire)) {
								...
								if (null != profilDelegataire) {
									AgentDto delegataire = new AgentDto(agentService.getAgentOptimise(
											listAgentsExistants, profilDelegataire.getDroit().getIdAgent()));
